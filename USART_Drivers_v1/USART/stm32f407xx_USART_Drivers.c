...
void USART_SetBaudRate(USART_RegDef_t *pUSARTx, uint32_t BaudRate)
{
  ...
  {
	   PCLKx =  RCC_GetPClk(); //Other USARTx peripherals (USART2,3; UART4,5)
  }
...
/******************************** Configuration of CR1******************************************/
	...
	//Not required to enable even parity because EVEN parity will be selected once you enable the parity control
	...
	for(uint32_t i = 0 ; i < len; i++)
	{
		...
		if(pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLEN_9BITS)
		{
			...
			if(pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE)
			{
				...
				pTxBuffer++;			//Because there is no parity bit (which will be set by hardware), there is 9 bits transmitted
				pTxBuffer++;			//Since 9 bits is 2 bytes of data, we have to increment the buffer twice.
			}
			else
			{
				//Parity bit is used in this transfer . so , 8bits of user data will be sent
				//The 9th bit will be replaced by parity bit by the hardware
				pTxBuffer++;			//Since only 8 bits will be read, buffer is only incremented once
			}
		}
		else
		{
			...
			pUSARTHandle->pUSARTx->DR = (*pTxBuffer  & (uint8_t)0xFF);
							...
							//Parity bit is used in this transfer . so , 8bits of user data will be sent
			pTxBuffer++;	//Since only 8 bits will be read, the buffer is only incremented once
...
/*Check for TXE flag */
	...
	//to check the state of TXE bit in the SR
	temp1 = pUSARTHandle->pUSARTx->SR & ( 1 << USART_SR_TXE);

	//to check the state of TXEIE bit in CR1
		...
		if(pUSARTHandle->RxBusyState == USART_BUSY_IN_RX)
		{
			...
			if(pUSARTHandle->RxLen > 0)
			{
				...
				else
				{
					...
					//check for USART_ParityControl control or not
					if(pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE)
					{
